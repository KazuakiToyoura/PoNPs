...
The PoNPs code was written by Kazuaki Toyoura in Nov. 2021. Dr. Katsuhiro Ueno has great contribution to the code development with fruitful discussions and many bug reports.
...
if __name__ == "__main__":
	...
	# When conc.csv is insufficient, ci_ are generated by linearly interpolating ci.
	# The initial concentrations in both adjacent elements to every hetero boundary are same.
	if c_smooth == 'true':
		print("Interpolate defect concentrations between cL and cR.")
		print("-"*30)
		print("Initial concentration profiles are created automatically.")
		# The intervals containing a hetero-boundary are set to zero.
		dx_m_B0_ = deepcopy(dx_m_)
		dx_m_B0_[0] = 0.0
		index_B = 0
		for i in n_elem:
			index_B = index_B + i
			dx_m_B0_[index_B] = 0.0
		# The normalized coordinate for each emelent.
		x=np.cumsum(dx_m_B0_)/np.sum(dx_m_B0_)
		x=np.append(0.0,x)
